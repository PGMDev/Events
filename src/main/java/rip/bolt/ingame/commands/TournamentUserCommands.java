...
import rip.bolt.ingame.format.rounds.format.FormatTournamentImpl;
...
public class TournamentUserCommands {
    ...
    public void currentScore(CommandSender sender, TournamentFormat format) {
        if (format instanceof FormatTournamentImpl) {
            ...
            Optional<TournamentFormat> parentOptional = Tournament.get().getTournamentManager().currentTournament();
    ...
    public void veto(CommandSender sender, Match match, TournamentTeamManager teamManager, TournamentFormat format, @Text String option) {
    ...
    public void rounds(CommandSender sender, TournamentFormat format) {
        ...
        sender.sendMessage(ChatColor.GOLD + "------- " + ChatColor.AQUA + header + ChatColor.GOLD + " -------");
        int round = 1;
