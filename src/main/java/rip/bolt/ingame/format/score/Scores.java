package rip.bolt.ingame.format.score;
...
import rip.bolt.ingame.utils.Pair;
...
public class Scores {
    ...
    public static Scores ofLegacy(Map<TournamentTeam, Integer> scores, Collection<? extends TournamentTeam> updated) {
        return of(scores.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, x -> new Score(x.getKey(), x.getValue()))), updated);
    }

    public static Scores of(Map<TournamentTeam, Score> scores, Collection<? extends TournamentTeam> updated) {
    ...
    public FormattedScore formattedScore(TournamentTeamManager teamManager) {
        if (scores.size() == 2) {
            ...
            // return teamManager.teamColour(topTwo.first.team()) + topTwo.first.team().teamName() + " " + topTwo.first.score() + ChatColor.GRAY + " - " + teamManager.teamColour(topTwo.second.team()) + topTwo.second.score() + " " + topTwo.second.team().teamName();
        }

        // size != 2, therefore return just the top score
        // Score highest = highestScore();
        // return ChatColor.GRAY + "Leader: " + teamManager.formattedName(highest.team()) + " (" + highest.score() + ")";
        return null;
    }
