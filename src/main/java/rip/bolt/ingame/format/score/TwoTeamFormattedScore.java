...
public class TwoTeamFormattedScore implements FormattedScore {
    ...
    public TwoTeamFormattedScore(TournamentTeamManager teamManager, Pair<Score, Score> pair, Collection<? extends TournamentTeam> justWon) {
    ...
    private String addPadding(String target, String otherString) {
        if (target.length() < otherString.length())
            for (int i = 0; i < otherString.length() - target.length(); i++)
                target = " " + target;
    ...
    public String condensed() {
        return teamManager.formattedName(topTwo.first.team()) + " " + ChatColor.WHITE + +topTwo.first.score() + ChatColor.GRAY + " - " + ChatColor.WHITE + topTwo.second.score() + " " + teamManager.formattedName(topTwo.second.team());
    }
