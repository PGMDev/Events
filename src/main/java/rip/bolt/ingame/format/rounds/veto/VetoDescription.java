...
public class VetoDescription implements RoundDescription {
    ...
    public BaseComponent roundInfo() {
        switch (vetoRound.phase()) {
            ...
            case WAITING:
                TextComponent waiting = new TextComponent(ChatColor.GREEN + "Deciding veto order with veto decider");
                waiting.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new BaseComponent[]{
                        tournamentFormat.currentRound().describe().roundInfo()
                }));
                return waiting;
            case RUNNING:
                TextComponent running = new TextComponent(ChatColor.GREEN + "Veto process is running now");
                running.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                        formattedVetosSoFar()));
                return running;
            case FINISHED:
                TextComponent finished = new TextComponent(ChatColor.GRAY + "Veto process has concluded");
                finished.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                        formattedVetosSoFar()));
                return finished;
        }
        //not gonna get here
    ...
    private BaseComponent[] formattedVetosSoFar() {
        ...
        if (history.size() > 0) {
            ...
            messages[vetoCount - 1] = new TextComponent(
                    fromHistory(vetoCount - 1, false, history.get(vetoCount - 1)));
        }
    ...
    public String countdown(TournamentTeam picking, VetoSettings.VetoType type) {
        return tournamentFormat.teamManager().formattedName(picking) + ChatColor.GRAY + " choose an option to " + actionWord(type);
    }
    ...
    public BaseComponent commandPrompt(VetoSettings.VetoType type) {
        return new TextComponent(actionWord(type) + ChatColor.GRAY + " with: " + ChatColor.GOLD + "/veto <number>");
