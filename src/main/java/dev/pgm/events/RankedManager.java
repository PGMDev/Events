...
public class RankedManager implements Listener {
  ...
  public boolean setupMatch() {
      ...
      pollTaskId =
            Bukkit.getScheduler()
                .scheduleSyncRepeatingTask(
                    Tournament.get(), pollTask, 15 * 20, 15 * 20); // every 15 seconds
      return false;
    }
    ...
    for (Team team : match.getTeams()) Tournament.get().getTeamManager().addTeam(team);
  ...
  public void onMatchFinish(MatchFinishEvent event) {
    for (Competitor winner : event.getWinners()) match.getWinners().add(winner.getNameLegacy());
    ...
    pollTaskId =
        Bukkit.getScheduler()
            .scheduleSyncRepeatingTask(
                Tournament.get(), pollTask, 30 * 20, 30 * 20); // every 30 seconds
  }
